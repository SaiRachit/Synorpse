...
async def process_user_command(user_input: str) -> str:
    ...
    if decision_type == "confirm":
        if "|" in resolved_query:
            command_part, desc_part = resolved_query.rsplit("|", 1)
    ...
    try:
        if decision_type == "generate" and "image" in resolved_query:
            ...
            response = await execute_with_reasoning(resolved_query)
            ...
            "recall_action", "read_screen", "read_screen_and_share", 
            "read_screen_and_document", "read_screen_and_search"
        ]):
            with timer(f"Automation: {decision[:30]}...", logger):
                results = await async_automation.execute(decisions)
                ...
                for r in results:
                    ...
                    if r_str.startswith('{') and r_str.endswith('}'):
                        try:
                            ...
                            parsed = _json.loads(r_str)
                            if 'analysis' in parsed:
                                formatted_responses.append(parsed['analysis'])
                            elif 'response' in parsed:
                                formatted_responses.append(parsed['response'])
                            else:
                                formatted_responses.append(r_str)
                        except:
                            formatted_responses.append(r_str)
                    else:
                        formatted_responses.append(str(r))
